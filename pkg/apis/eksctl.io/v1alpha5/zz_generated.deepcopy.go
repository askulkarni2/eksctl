	...
	*out = *in
	...
	if in.KubeletExtraConfig != nil {
		...
		*out = new(NodeGroupKubeletConfig)
		if **in != nil {
			in, out := *in, *out
			*out = make(map[string]Copyable, len(*in))
			for key, val := range *in {
				if val == nil {
					(*out)[key] = nil
				} else {
					(*out)[key] = val.DeepCopyCopyable()
				}
			}
		}
	}
...
func (in NodeGroupKubeletConfig) DeepCopyInto(out *NodeGroupKubeletConfig) {
	{
		in := &in
		*out = make(NodeGroupKubeletConfig, len(*in))
		for key, val := range *in {
			if val == nil {
				(*out)[key] = nil
			} else {
...
// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new NodeGroupKubeletConfig.
